...
def get_dataset(
  ...
  ds = datasets.DatasetDict({"test": datasets.Dataset.from_list(rows)})
...
def main() -> None:
  ...
  # Force the slow tokenizer to avoid protobuf issues in older MetricX
  # environments when Transformers tries to build a fast tokenizer from the
  # SentencePiece model.
