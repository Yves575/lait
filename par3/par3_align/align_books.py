...
parser.add_argument('--output_file', default=None, help="Optional fixed output pickle filename. Defaults to <book>.pkl.")
...
def get_output_file(book):
    """Return the pickle filename for a book."""
    return args.output_file or f"{book}.pkl"
...
def main():
    ...
    for book, _ in tqdm.tqdm(book_size):
        ...
        if not args.long_books and book in ["the_count_of_monte_cristo_fr", "les_miserables_fr", "romance_of_the_three_kingdoms_zh"]:
            continue
        output_file = get_output_file(book)
        # alignment data stored in output_file, skip if exists
        if os.path.exists(os.path.join(args.dataset, book, output_file)) and not args.overwrite:
            continue
        ...
        with open(os.path.join(args.dataset, book, output_file), "wb") as f:
